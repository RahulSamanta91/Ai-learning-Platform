        ...
        <nav className="flex gap-2 overflow-x-auto whitespace-nowrap">
          {tabs.map((tab) => (
            <button
              ...
              className={`relative pb-4 px-2 md:px-6 text-sm font-semibold transition-all duration-200 cursor-pointer ${
                activeTab === tab.name
                  ? "text-emerald-600"
            ...
            >
              ...
              {activeTab === tab.name && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-linear-to-r from-emerald-500 to-teal-500 rounded-full shadow-lg shadow-emerald-500/20" />
              )}
              {activeTab == tab.name && (
