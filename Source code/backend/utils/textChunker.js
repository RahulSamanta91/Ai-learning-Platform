 ...
 * @param {string} query - Search the query
...
export const findRelevantChunks = (chunks, query, maxChunks = 3) => {
  ...
  // Extract and clean keywords from query
  ...
  const scoredChunks = chunks.map((chunk, index) => {
    ...
    // scoreeach query word
    for (const word of queryWords) {
      // Extra word match(higher score
      const extraMatches = (
        content.match(new RegExp(`\\b${word}\\b`, "g")) || []
      ).length;
      score += extraMatches * 3; // Extra weight for multiple matches

      // Partial match (lower score)
      const partialMatches = (content.match(new RegExp(word, "g")) || [])
        .length;
      score += Math.max(0, partialMatches - extraMatches) * 1.5;
    }
    ...
    if (uniqueWordsFound > 1) {
      score += uniqueWordsFound * 2; // Bonus points
    }

    // Normalize by content length
    const normalizedScore = score / Math.sqrt(contentWords);

    // Small bounus for earlier chunks (assuming they are more relevant)
  ...
  return scoredChunks
    ...
    .sort((a, b) => {
      ...
      if (b.matchedWords !== a.matchedWords) {
        return b.matchedWords - a.matchedWords; // Secondary sort by matched words
      }
