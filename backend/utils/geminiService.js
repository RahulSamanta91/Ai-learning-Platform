...
export const generateFlashcards = async (text, count = 10) => {
  const prompt = `Generate exactly ${count} educational flashcards from the following from the following text.
  Format each flashcard as:
  Q: [Clear, specific question]
  A: [Concise, accurate answer]
  D: [Difficulty level : easy, medium, or hard]
  
  Seperate each flashcard with "___"
 ...
 * @param {string} questions - User Questions
...
export const chatWithContext = async (question, chunks) => {
  ...
  const prompt = `Based on the following context from a document, analyze the context and answer the user's question. If the answer is not present in the context, clearly say so.
...
/**
 * Expalin a specific concept from document
